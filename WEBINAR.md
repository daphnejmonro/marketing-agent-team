...
### 1. Clone the repo
```bash
git clone https://github.com/daphnemonro/marketing-agent-team.git
...
### 3. Set your API key
```bash
export ANTHROPIC_API_KEY="sk-ant-..."  # Your Anthropic API key from console.anthropic.com
```
...
(We use `--max=20` to keep it fast for the demo. Full-site crawl uses `--max=2000` and takes ~45 min.)
...
2. **Add your own API key:** If you have an Ahrefs key, uncomment those env vars and run `hosting-seo-agent-system.js`
