...
const titlePage = [
  ...
  new Paragraph({alignment:AlignmentType.CENTER, children:[run("Prepared by: Daphne Monro, Head of Website & Content",{size:22,color:GREY_TEXT,font:"Arial"})], ...sp(0,60)}),
  new Paragraph({alignment:AlignmentType.CENTER, children:[run("Date: June 2026",{size:22,color:GREY_TEXT,font:"Arial"})], ...sp(0,60)}),
  ...
  new Table({
    ...
    rows:[new TableRow({children:[
      ...
      new TableCell({ borders:noBdr, width:{size:Math.ceil(CONTENT_W/2),type:WidthType.DXA}, margins:{top:80,bottom:80,left:120,right:0}, children:[
        ...
        para(run("Date: June 2026",{size:20})),
      ]}),
    ]})]
  }),
  spacer(200),
...
const outPath = path.join(__dirname, "Agent-Setup-Security-Guide.docx");

Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync(outPath, buf);
  console.log("");
  console.log("✅  All done! Your document is ready.");
  console.log("");
  console.log("    File:  Agent-Setup-Security-Guide.docx");
  console.log("    Saved here:  " + outPath);
  console.log("");
  console.log("    Double-click it to open in Word.");
  console.log("");
}).catch(err => {
  console.log("");
  console.log("❌  Sorry, something went wrong creating the document.");
  console.log("    Please send this message to whoever set this up:");
  console.log("");
